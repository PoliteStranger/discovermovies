...
namespace API_DiscoverAlgorithm
{
    public class ApiOps
    {
        ...
        public static async Task<string> RunApiPersonCredits(int personId)
        {
            using (var client = new HttpClient())
            {
                client.BaseAddress = new Uri("https://api.themoviedb.org");
                ...
                HttpResponseMessage response = await client.GetAsync("3/person/" + personId.ToString() + "/movie_credits?api_key=4e00a65a5badd5659a449269be8fcd80");
                //Console.WriteLine("URL: " + client.BaseAddress + "3/search/movie/" + movieId + "?api_key=4e00a65a5badd5659a449269be8fcd80");
                if (response.IsSuccessStatusCode)
                {
                    //Console.WriteLine("Got Response!");
